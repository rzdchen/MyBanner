...
/**
 * MyViewPager的适配器，为页面填充数据
 */
public class MyBannerAdapter extends PagerAdapter {
    private Context mContext;
    private SparseArray<MyBannerViewHolder> mCachedViews = new SparseArray<>();
    private MyBanner.OnBannerClickListener mBannerClickListener;
    private IBindAdapter mBindAdapter;
    private List<? extends MyBannerMo> models;
    /**
     * 是否开启自动轮
     */
    ...
    public MyBannerAdapter(@NonNull Context mContext) {
    ...
    private void initCachedView() {
        ...
        for (int i = 0; i < models.size(); i++) {
            MyBannerViewHolder viewHolder = new MyBannerViewHolder(createView(LayoutInflater.from(mContext), null));
    ...
    public Object instantiateItem(@NonNull ViewGroup container, int position) {
        ...
        MyBannerViewHolder viewHolder = mCachedViews.get(realPosition);
    ...
    protected void onBind(@NonNull final MyBannerViewHolder viewHolder, @NonNull final MyBannerMo bannerMo, final int position) {
    ...
    public static class MyBannerViewHolder {
        private SparseArray<View> viewHolderSparseArr;
        View rootView;

        MyBannerViewHolder(View rootView) {
